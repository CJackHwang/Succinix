...
export async function runFilesystem(ctx: TestContext): Promise<void> {
  ...
  const nodeCwd = String(fs3.stdout ?? '').trim();
  verdict(
    term,
    'Filesystem',
    'node child cwd is a real execution-world path',
    fs3.ok && fs3.runtime === 'node' && nodeCwd.startsWith('/'),
    nodeCwd,
  );
  ...
  const cdSync1 = await client.terminal('mkdir -p /workspace/.succinix-selftest-cwd && cd /workspace/.succinix-selftest-cwd');
  ...
  const syncedNodeCwd = String(cdSync3.stdout ?? '').trim();
  verdict(
    term,
    'Filesystem',
    'cd syncs session cwd (node child cwd follows)',
    cdSync1.ok &&
      String(cdSync2.cwd ?? '') === '/workspace/.succinix-selftest-cwd' &&
      syncedNodeCwd.endsWith('/.succinix-selftest-cwd'),
    `session=${String(cdSync2.cwd)} node=${syncedNodeCwd}`
  );
  ...
  verdict(
    ...
    cwdFile.trim() === '/workspace/.succinix-selftest-cwd',
  ...
  verdict(
    ...
    cdBad.ok === false && String(cwdAfterBad.cwd ?? '') === '/workspace/.succinix-selftest-cwd',
  ...
  await client.terminal('cd /workspace && rmdir /workspace/.succinix-selftest-cwd');
}
