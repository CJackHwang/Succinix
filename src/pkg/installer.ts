...
export async function installPackage(ctx: PkgContext, rawName: string): Promise<ActionResult> {
  ...
  const r = await ctx.client.terminal(`succinix pkg install npm:${q(name)}`, TIMEOUT.install.opts, TIMEOUT.install.wait);
  const out = String(r.stdout ?? '').trim();
  if (r.ok) {
...
export async function removePackage(ctx: PkgContext, rawName: string): Promise<ActionResult> {
  ...
  if ((await listNpm(ctx.wc)).some((p) => p.name === name)) {
    const r = await ctx.client.terminal(`succinix pkg remove npm:${q(name)}`, TIMEOUT.remove.opts, TIMEOUT.remove.wait);
    const out = String(r.stdout ?? '').trim();
    if (r.ok) {
