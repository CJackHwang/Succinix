...
async function s3(h) {
  const checks = [];
  // 1. 通过执行世界 service manager 启动 tinbase（首次含 npm install，重活）
  const ds = await h.run('succinix service start tinbase', 200000);
  check(checks, 'succinix service start tinbase (wasm)', ds.handled === true && ds.ok === true, ds.output ? ds.output.slice(-120) : '');
...
`;
  ...
  // 3. stop
  const st = await h.run('succinix service stop tinbase');
  check(checks, 'succinix service stop tinbase', st.handled === true && st.ok === true, st.output ? st.output.trim().slice(0, 120) : '');

  // 4. 再起（同会话 tinbase 已装，跳过 install）
  const ds2 = await h.run('succinix service start tinbase', 120000);
  check(checks, 'succinix service restart tinbase', ds2.handled === true && ds2.ok === true, ds2.output ? ds2.output.slice(-120) : '');
...
`;
  ...
  // 清理
  await h.run('succinix service stop tinbase');
...
async function s4(h) {
  const checks = [];
  // 1. enable tinbase
  const en = await h.run('succinix service enable tinbase');
  check(checks, 'succinix service enable tinbase', en.handled === true && en.ok === true, en.output ? en.output.trim().slice(0, 120) : '');

  // 2. 刷新 → boot 自动拉起
  await h.reloadAndWait(180000);
  const st = await h.run('succinix service status tinbase');
  // 诊断：进程表来自同一执行世界注册表。
  ...
  const diagProcs = (diagPs.processes ?? [])
    ...
    .join(' || ') || 'none';
  const diagDetail = `state=${st.output.replace(/\x1b\[[0-9;]*m/g, '').trim().split('\n').find((l) => l.includes('Active:')) || '?'}; procs=${diagProcs}`;
  check(checks, 'boot autostart pulls tinbase up', st.handled === true && st.output.includes('Active: active (running)'), diagDetail.slice(0, 400));

  // 3. disable → 刷新不再拉起
  const dis = await h.run('succinix service disable tinbase');
  check(checks, 'succinix service disable tinbase', dis.handled === true && dis.ok === true, dis.output ? dis.output.trim().slice(0, 120) : '');
  await h.reloadAndWait(120000);
  const st2 = await h.run('succinix service status tinbase');
  check(checks, 'disabled service not autostarted', st2.handled === true && st2.output.includes('Active: inactive'), st2.output ? st2.output.replace(/\x1b\[[0-9;]*m/g, '').trim().slice(0, 160) : '');
  return checks;
}
