...
- [能力说明](FEATURES.zh-CN.md)：判断它能不能解决你的问题。
- [运行时说明](LANGUAGES.zh-CN.md)：确认语言、包和已知限制。
- [接入说明](SDK.zh-CN.md)：把 Succinix 接入 Cordis 应用。
- [第三方插件](PLUGIN.zh-CN.md)：写使用 Succinix 的 Cordis 插件。
- [迁移说明](MIGRATION.zh-CN.md)：从旧接入方式升级。
- [协议](PROTOCOL.zh-CN.md) 与 [契约](cordis-contract.zh-CN.md)：只在实现宿主、传输层或严格兼容时阅读。
