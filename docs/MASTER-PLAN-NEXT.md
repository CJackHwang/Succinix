...
- 本地收口命令（O13 落地前手动执行）：`npm run typecheck && npm run lint &&
  npm run test && npm run build && npm run build:engine-package`。
- 每项重构保持：tsc 0 / lint 0 / vitest 334+ / `?test=1 >=71` 且 0 failed。
...
| --- | --- | --- | --- | --- | --- |
| O7 | 拆分 `src/engine/python-daemon.ts` | P3 | 无 | TBD | `[ ]` |
| O8 | 拆分 `src/services.ts` | P3 | 无 | TBD | `[ ]` |
| O9 | `src/terminal/` 子模块化 | P3 | 无 | TBD | `[ ]` |
| O10 | 拆分 `src/pkg.ts` | P3 | 无 | TBD | `[ ]` |
| O11 | 拆分 `scripts/scenarios.mjs` | P3 | 无 | TBD | `[ ]` |
| R5 | 同页多实例宿主行为补 e2e | P2 | 无 | TBD | `[ ]` |
| R6 | 场景套件接入 push CI | P2 | 无 | TBD | `[ ]` |
| O12 | 文件规模审计脚本 | P2 | 无 | TBD | `[ ]` |
| O13 | 一行本地验收命令 | P2 | 无 | TBD | `[ ]` |
| O14 | 文档/引用完整性检查 | P3 | 无 | TBD | `[ ]` |

## 4. 详细任务

### TASK-O7：`src/engine/python-daemon.ts` 拆分（578 行）
...
- commit：`refactor(O7): split python daemon loader/rpc/process-bridge`

### TASK-O8：`src/services.ts` 拆分（497 行）

- 建议结构：`src/services/{io,ports,lifecycle,registry}.ts`
- 职责拆分：服务文件 I/O、端口视图、生命周期、注册表。
- 验收：O 系列门禁全过；service/ports/db 场景不回归。
- commit：`refactor(O8): split services into domain modules`

### TASK-O9：`src/terminal/` 子模块化（session 486 行 / boot 408 行）

- 建议：session 的输入解析/历史/补全，boot 的步骤编排/log 输出按需拆子模块。
- 条件：仅在继续扩展终端功能时执行，避免为拆分而拆分。
- 验收：O 系列门禁全过；SDK `./terminal` 导出与行为不变。
- commit：`refactor(O9): modularize terminal session/boot internals`

### TASK-O10：`src/pkg.ts` 拆分（401 行）

- 建议结构：`src/pkg/{metadata,registry,installer}.ts`
- 验收：O 系列门禁全过；`pkg`/语言验证场景不回归。
- commit：`refactor(O10): split pkg into metadata/registry/installer`
...
- commit：`chore(O12): add file size audit`
...
- commit：`chore(O13): add single-command quality gate`
...
- commit：`chore(O14): add docs integrity check`
...
- commit：`test(R5): cover same-page instance routing end to end`
...
- commit：`ci(R6): run scenario suite on push`

## 5. 建议执行顺序

O12/O13 先行（工具化），R6 次之（防回归），R5 与 O7/O8/O10/O11 可并行，
O9 延后到终端特性扩展时再执行，O14 可随任何文档变更立即落地。
