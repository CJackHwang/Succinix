...
- System information & login banner: `uname` reports the honest browser-native system identity — summary line `WebUnix 0.1.0 js-runtime+webcontainer <api-version> <arch>` (kernel identified as `js-runtime+webcontainer`, never impersonating a Linux kernel), `-a` all fields with hostname/OS, `-r` the `@webcontainer/api` runtime version, `-m` the UA-derived architecture (`unknown` when absent) — and `motd` views/edits the persisted login banner at `/etc/webunix.motd` (`motd <text>` sets it, `motd reset` restores the default welcome line, printed at every boot).
