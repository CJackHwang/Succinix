...
// 非安全边界（UI 展示 / 查询过滤用）。
export function dispatchPs(req: CommandRequest): void {
...
export function dispatchKill(req: CommandRequest): void {
  const inst = instanceOf(req);
...
export function dispatchResetInstance(req: CommandRequest): void {
  const inst = instanceOf(req);
  const killed: number[] = [];
