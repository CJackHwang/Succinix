...
// isExcludedPath 排除 .succinix-*/tinbase 二进制 DB 目录（D5）。
...
describe('persist instance scope (D4/D5)', () => {
  it('instance-scoped snapshot keeps own roots + shared workspace, drops other instances', async () => {
    ...
    expect(res.meta.fileCount).toBe(3); // shared.txt + own env + own notes（tinbase 已排除）
    ...
    expect(dst.has('/workspace/.succinix-a/tinbase/db')).toBe(false);
  ...
  it('default context keeps whole-FS snapshot (unchanged)', async () => {
    ...
    expect(dst.has('/workspace/.succinix-a/tinbase/db')).toBe(false); // D5 通用排除
  });

  it('D5: instance tinbase dirs are excluded, own state files are not', () => {
    expect(isExcludedPath('/workspace/.succinix-c-1/tinbase')).toBe(true);
    expect(isExcludedPath('/workspace/.succinix-c-1/tinbase/db')).toBe(true);
    expect(isExcludedPath('/workspace/.succinix-c-1/etc/succinix.env')).toBe(false);
    expect(isExcludedPath('/workspace/.tinbase/data')).toBe(true);
  });
});
